...
module  RV32core(
    ...
    wire refetch,Predict_PC_Branch;
    wire [31:0] Predict_PC, After_predict_PC,Collide_PC;

    Predict_branch_ctrl Predict_branch_ctrl(
        .clk(debug_clk),
        .rst(rst),
        .PC_EN_IF(PC_EN_IF),
        ...
        .PC_ID(PC_ID),
        .PC_IF(PC_IF),
        .jump_PC_ID(jump_PC_ID),
        .Branch(Branch_ctrl),
        .refetch(refetch),
        .next_PC_IF(next_PC_IF)
    );
    // branch prediction, 选择预测的PC或实际的PC
    // MUX2T1_32 mux_PC_IF(.I0(PC_4_IF),.I1(Predict_PC),.s(Predict_PC_Branch),.o(After_predict_PC));
    
    // MUX2T1_32 mux_IF(.I0(After_predict_PC),.I1(Collide_PC),.s(refetch),.o(next_PC_IF));
